...
from bokeh_config import Config

conf = Config()

# Set up data
data_days = list(range(1, conf.BARS_MAX_DAYS + 1))
data_bugs = [random.randint(1, 100) for i in range(1, conf.BARS_MAX_DAYS + 1)]

x = data_days[:conf.BARS_DEFAULT_DAYS]
y = data_bugs[:conf.BARS_DEFAULT_DAYS]
...
# Set up widget
bar_slider = Slider(title='Number of Days', value=conf.BARS_DEFAULT_DAYS, start=1, end=100, step=1)
